...
- Apply Clean Architecture for new and refactored implementations based on discussion #26:
  - Use a domain-centered module structure: `presentation/`, `application/`, `domain/`, `infrastructure/`.
  - Keep dependency direction: Presentation → Application → Domain, Infrastructure implements Domain/Application.
  - One endpoint = one UseCase (Application layer).
  - Domain/Application must not depend on NestJS, Prisma, HTTP, JWT/OAuth, etc.
