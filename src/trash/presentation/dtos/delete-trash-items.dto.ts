...
export class DeleteTrashItemDto {
...
export class DeleteTrashItemsDto {
  @ApiProperty({
    type: [DeleteTrashItemDto],
    minItems: 1,
    uniqueItems: true,
    example: [
  ...
  @IsArray()
  @ArrayNotEmpty()
  @ArrayUnique((item: DeleteTrashItemDto) => `${item.itemType}:${item.id}`)
  @ValidateNested({ each: true })
  @Type(() => DeleteTrashItemDto)
  items: DeleteTrashItemDto[];
}
