...
export class ClipsController {
  ...
  createClip(
    ...
    return this.clipsService.createClip(req.user.userId, dto, file);
  }

  // 삭제되지 않은 내 클립을 단건으로 조회한다.
  @Get(':id')
  getClip(@Request() req: { user: AuthContext }, @Param('id') id: string) {
    return this.clipsService.getClipById(req.user.userId, id);
  }
  ...
  updateClip(
    ...
    return this.clipsService.updateClip(req.user.userId, id, dto, file);
  }

  // 클립을 즉시 제거하지 않고 deletedAt만 기록한다.
  @Delete(':id')
  deleteClip(@Request() req: { user: AuthContext }, @Param('id') id: string) {
    return this.clipsService.deleteClip(req.user.userId, id);
  }
}
