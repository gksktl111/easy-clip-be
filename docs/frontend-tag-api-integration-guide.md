...
폴더가 없거나 미소유·삭제 상태면 `404 Not Found`, 잠긴 폴더면 `403 PROJECT_LOCKED`, 접근 폴더의 Free 요청이면 `403 FEATURE_NOT_AVAILABLE`입니다.
...
| `403 FEATURE_NOT_AVAILABLE` | 유효 Free의 태그 전용 API 요청 | 태그 변경을 취소하고 Pro 안내를 표시합니다. 빈 배열로 재전송하지 않습니다. |
| `403 PROJECT_LOCKED` | 잠긴 폴더의 태그 요청 | 기존 폴더 잠금 안내를 표시합니다. |
| `404 Not Found` | 폴더·태그·클립이 없거나 미소유·삭제됨 | 최신 목록을 다시 불러오고, 필요하면 이전 화면으로 이동시킵니다. |
